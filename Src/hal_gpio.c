...
void My_HAL_GPIO_PA4_DAC (void)
{
    GPIOA->MODER |= ((0x1 << 8) | (0x1 <<9)); // Set to analog mode
    GPIOA->PUPDR &= ~((0x1 << 8) | (0x1 <<9)); // No pull up or pull down resistor;
    DAC1->SWTRIGR |= DAC_SWTRIGR_SWTRIG1; // Enable software trigger for DAC channel 1
